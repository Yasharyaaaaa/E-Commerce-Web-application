...
const Home = () => {
  ...
  // Reset pagination when allProducts changes (fresh load) — use a ref to
  // track the previous length so we only reset when the list is replaced
  const prevProductsLengthRef = useRef(0);
  useEffect(() => {
    if (allProducts.length !== prevProductsLengthRef.current) {
      prevProductsLengthRef.current = allProducts.length;
